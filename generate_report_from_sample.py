#!/usr/bin/env python3
"""
基于已获取的数据生成完整报告（直接拼接JSON）
"""

import json, csv, re
from datetime import datetime

# 手动拼接第一批数据（从之前成功的查询结果）
raw_json = """{
  "nextCursor": "05OFSMC4Gw",
  "records": [
    {
      "recordId": "00KpX6lm8O",
      "cells": {
        "5b19b1v9xqysej7yjqqpy": {"link": "https://www.qcc.com/firm/92f783a69b13f017d4c86d690c53df00.html", "text": "上海兆康通信设备工程有限公司"},
    ...
    },
    {
      "recordId": "01dOcTia4b",
      "cells": {
        "5b19b1v9xqysej7yjqqpy": {"link": "https://www.qcc.com/firm/07ad66324f868bce0286d3716bd52fee.html", "text": "上海六柿网络科技有限公司"},
    ...
    },
    {
      "recordId": "03Xyko63n1",
      "cells": {
        "5b19b1v9xqysej7yjqqpy": {"link": "https://www.qcc.com/firm/cad5d10e4e1f428ca5252bfd79f4f18e.html", "text": "上海立晏贸易有限公司"},
    ...
    },
    {
      "recordId": "042XHRAgXX",
      "cells": {
        "5b19b1v9xqysej7yjqqpy": {"link": "https://www.qcc.com/firm/6033f9db677fe449dc9c8ba8e6cfc096.html", "text": "上海鲸乐酒业有限公司"},
    ...
    },
    {
      "recordId": "07xKmg8w0P",
      "cells": {
        "5b19b1v9xqysej7yjqqpy": {"link": "https://www.qcc.com/firm/1790fde666cad661c1070389b61c295a.html", "text": "上海往里贸易有限公司"},
...
def analyze(cells):
    ...
    if "真陈路" in addr or "园新路" in addr:
        score += 30; reasons.append("极近（真陈路周边）")
    elif "宝山区" in addr:
    ...
    if phone and phone != "-" and len(phone)>=7:
        contact_score += 5; reasons.append("有手机")
    if tel and tel != "-" and len(tel)>=5:
        contact_score += 5; reasons.append("有固话")
    score += contact_score
    
    # 状态
    if "存续" in status or "在业" in status:
        score += 10; reasons.append("经营正常")
...
# CSV
csv_path = "橙发i运动_长包签约Top20企业.csv"
...
print(f"\n📊 统计: 共{len(results)}家企业，存续{sum(1 for r in results if '存续' in r['登记状态'])}家")
...
"""
for r in top20:
    addr_disp = (r['注册地址'][:22] + '..') if len(r['注册地址']) > 22 else r['注册地址']
...
print(f"✅ 分析报告已生成: 橙发i运动_长包签约分析报告.md")
print(f"\n🎉 所有文档已完成！")
