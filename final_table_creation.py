#!/usr/bin/env python3
"""
获取企业数据并写入钉钉多维表格（使用已有Base）
"""

import subprocess, json, time, re
from datetime import datetime

BASE_ID = "l6Pm2Db8D4Xdo9lYUGL3lXy48xLq0Ee4"  # 源数据Base
TABLE_ID = "d7FEhge"  # 源数据Table
TARGET_BASE_ID = "7QG4Yx2JpLZb15rBFB5gNrGdJ9dEq3XD"  # 目标Base（OpenClaw测试表格）
...
def fetch_all_records():
    ...
    while True:
        cmd = ["mcporter", "call", "dingtalk-ai-table.query_records",
               f"baseId:{BASE_ID}", f"tableId:{TABLE_ID}", f"limit:50"]
        if cursor:
...
def create_table_in_target_base(results):
    """在已有Base中创建表格并插入数据"""
    print("\n" + "="*60)
    ...
    try:
        ...
        for field in fields:
            ...
            if 'property' in field:
                add_cmd.append(f"property:{json.dumps(field['property'])}")
            
            res = subprocess.run(add_cmd, capture_output=True, text=True, timeout=30)
            if res.returncode == 0:
                print(f"  ✅ {field['fieldName']}")
            else:
                print(f"  ⚠️  {field['fieldName']} - {res.stderr}")
        
        time.sleep(1)
        
        # 插入数据（批量）
        print(f"\n📥 正在插入 {len(results)} 条数据...")
        for idx, r in enumerate(results, 1):
            ...
            rec_res = subprocess.run(add_cmd, capture_output=True, text=True, timeout=30)
            if rec_res.returncode != 0:
                print(f"  ❌ 第{idx}条失败: {rec_res.stderr}")
            elif idx % 10 == 0:
        ...
        return {
            ...
            "url": f"https://alidocs.dingtalk.com/i/nodes/{TARGET_BASE_ID}"
        }
...
def main():
    ...
    # 5. 创建钉钉表格
    table_info = create_table_in_target_base(top20)
    ...
    if table_info:
        print(f"\n📋 结果表格: {table_info['table_name']}")
        print(f"🔗 查看: {table_info['url']}")
    else:
        print("\n⚠️  表格创建失败，数据已保存在内存中")
